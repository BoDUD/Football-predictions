...
def build_payload(base: Path, count: int = 6, *, one_week: bool = False) -> dict:
    ...
    payload["history_ledger_binding"] = forward_validation.build_history_ledger_binding(
        payload,
        archive_version_hash="sha256:" + "7" * 64,
        record_commitment_hash="sha256:" + "8" * 64,
    )
    return payload


def pending_micro_ledger(base: Path) -> dict:
    payload = build_payload(base, 1)
...
class ForwardValidationTests(unittest.TestCase):
    def test_memory_record_review_evaluate_replays_archived_micro_ledger(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                report["history_ledger_binding"]["archive_version_hash"],
    ...
    def test_v2_scores_committed_rows_but_never_claims_external_time_anchor(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            report = forward_validation.evaluate(build_payload(Path(temporary), 6))
    ...
    def test_v2_evaluation_rejects_freely_constructed_unarchived_input(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            value.pop("history_ledger_binding")
            with self.assertRaisesRegex(
                forward_validation.ForwardValidationError, "history ledger binding"
            ):
                forward_validation.validate_input(value)

    def test_history_ledger_delete_reorder_copy_and_truncate_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            value = build_payload(Path(temporary), 2)
            ...
            truncated["history_ledger_binding"]["market_commitments"] = truncated[
                "history_ledger_binding"
            ]["market_commitments"][:-1]
            attacks["truncate"] = truncated
            reordered = deepcopy(value)
            reordered["history_ledger_binding"]["market_commitments"].reverse()
            attacks["reorder"] = reordered
            copied = deepcopy(value)
            copied["history_ledger_binding"]["market_commitments"].append(
                deepcopy(copied["history_ledger_binding"]["market_commitments"][0])
            )
            attacks["copy"] = copied
            for name, attacked in attacks.items():
                with self.subTest(name=name):
                    ...
                    with self.assertRaisesRegex(
                        forward_validation.ForwardValidationError,
                        "market commitments do not replay",
                    ):
    ...
    def test_same_cohort_id_replacement_and_late_queue_addition_are_rejected(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(
                ...
                forward_validation.validate_input(replaced)
            ...
            with self.assertRaisesRegex(
                ...
                forward_validation.validate_input(late)
    ...
    def test_missing_outcome_and_one_cluster_are_explicit_blockers(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            value = build_payload(Path(temporary), 6, one_week=True)
            settlement = value["settlements"][0]
            settlement.update(
                {
                    "status": "pending",
        ...
        self.assertEqual(
            ...
            [value["settlements"][0]["observation_id"]],
        )
    ...
    def test_protocol_override_is_experimental_and_report_is_deterministic(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            value = build_payload(Path(temporary), 4)
