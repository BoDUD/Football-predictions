...
当前发布版本：**3.13.0**（2026-08-09）。包/Skill 版本只描述本仓库发行；模型、归档、候选审计和调度策略的 artifact schema/policy 版本独立管理，不随发行版本自动改写。

3.13.0 保留严格实盘主推 Gate，同时为每个新归档确定一个不可手工覆盖的非金额“评测主推”；评测主推独立统计命中率，不计 ROI。赛后会冻结 `review-training-sample/1.0.0`，只有 cohort 关闭后才能导出供下一模型版本训练，旧归档不回填。
...
主表的 8 列与严格实盘主推定义保持不变。主表下的“评测主推与发布状态”区不是第 9 列：3.13+ 新归档每场都有一个不可手工指定的 `评测主推`，按严格正式、counterfactual shadow、可执行强制评测、冻结全场 1X2 四级选择，只计独立命中率。只有严格正式方向可进入主表 `主推`、金额和 ROI。非发布 Gate 全过时还会显示唯一 `◇ 观察首选` 及 EV/edge；其他情况保留数据/价值/政策/安全阻断。旧归档不使用现模型回填评测或观察方向。
...
3.13+ 复盘另存 `review-training-sample/1.0.0`，把全部候选概率与真实 FT/HT/角球目标一起冻结。`stats` 分开报告强制评测命中率、严格正式主推胜率/ROI、各选择层级以及全候选校准。样本只能在所属 cohort 关闭后通过 `python -m scripts.review_training_export` 导出到新的 staging bundle，供下一模型版本训练；同一 active cohort 内不得更新模型，旧复盘不补造样本。
