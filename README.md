...
默认输出为“简单图片 + 正常分析文字”。初盘和临场复查统一使用同一张 8 列表格：`编号`、`时间`、`赛事`、`主队 vs 客队`、`主推`、`总进球`、`半全场`、`波胆`；日期、标题与阶段副标题全部从归档派生，同一张图禁止混合阶段、比赛日期或同一场的两个版本。图片中的总进球只展示概率最高的一个区间及其领先差；半全场与波胆以概率最高的半场状态为共同根节点，完整覆盖其后的全场胜、平、负三路，并为每一路配对该分支概率最高的真实比分路径。展示顺序固定为延续结果优先、其余结果按 H/D/A 枚举，因此半场平时始终是 `平平`、`平胜`、`平负`；各自条件概率另行显示，不把结构顺序冒充概率排名。系统内部仍完整计算半场、1X2、全部总进球区间、BTTS、EV/edge 与证据审计，并在配套文字或机器归档中保留，不因图片精简而跳过分析。正式主推后标红色 `★`；通过诊断资格的独立观察，或有效联合模型在无正式主推时派生的 `◇ 模型首选`，可用 `◇`，两者均明确不计主推战绩。严禁独立排列半全场与比分后暗示对应，也严禁按终场方向硬配比分或手填 fallback；没有通过校验并绑定指定 `analysis_stage` 与版本哈希的联合后验 artifact 时，对应单元格必须显示 `数据不足`。
...
正常初盘与临场归档现在默认要求有效的 `--joint-scenario-file`；`record` 会以完整分析模式拒绝缺失、过期或身份不一致的联合路径，避免先生成一张“数据不足”的完成图。有效联合模型存在但正式盘口门槛未通过时，图片会把统一模型的 1X2 首位显示为 `◇ 模型首选（不计战绩）`，同时继续展示同一联合分布导出的一个总进球区间，以及领先半场状态下完整的三路半全场—代表波胆配对；它不计正式主推、注额或收益。旧冻结版本若当时没有联合制品，仍如实显示 `数据不足`，不得用赛果或其他版本回填。
