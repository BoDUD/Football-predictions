...
面向 Codex 的足球赛前分析、T−30 临场复查和赛后复盘 Skill。数据主要来自 Titan007，覆盖亚盘、大小球、总进球区间、双方进球、角球大小/让球、欧赔、半场、半全场、阵容和伤停。
...
- `stability-v1` 不再把普通方向的 EV ≥ 8% 作为主推硬门槛。候选必须先通过安全校验：正 EV、正模型边际、中高数据质量、完整且可执行的当前市场，以及该市场要求的阵容、基本面、机会质量、角球画像或尾部风险证据。
- 通过安全校验的候选按综合置信度排序，唯一的 `rank=1` 成为主推；如果没有安全候选，仍可明确不下注，不能为了每场都有主推而强行选择。
...
- `stability-v1` 只从启用后的新分析向前执行，不追认历史观察项为主推，也不重算旧战绩。比赛 2912847 仅用于前向规则审计，历史归档仍保持无主推。

## 小样本 Guardrail

- 普通方向使用 `stability-v1` 安全校验和综合置信排序，不再套用统一的 EV ≥ 8% 硬门槛；EV 和模型边际仍必须为正，且不能代替完整市场与专属证据。
