...
2. Aggregate the path cells into genuine `(HT/FT × full-time score)` events and rank them by joint probability. The canonical archive keeps the first two as its internal Top-2 audit. For public display, take the highest-probability half-time result as the common root, include its three mutually exclusive full-time branches, and select the highest-probability genuine score path within each branch. Put the continuity branch first and enumerate the remaining outcomes in canonical H/D/A order; show `P(FT result | selected HT result)` explicitly so the structural order is not read as a probability rank. Each displayed row contains the HT/FT outcome, its conditional branch probability, the representative full-time score, and that score path's genuine joint probability.
...
5. Label every displayed joint event `高方差参考（不计主推）`. The three branch representatives and their hit or miss never enter primary accuracy, profit, or ROI.
...
- In normal pre-match output, 0-0 appears only when it is the representative full-time score path selected for one of the leading half-time state's three branches. Show that path's joint probability; do not append the independent 0-0 marginal, rank, odds, or EV.
...
- Pass `--joint-scenario-file <joint-scenarios.json>` to bind the immutable path posterior and its canonical global Top-2 audit. The archive validator must reproduce the ranked events and reject a wrong fixture/version, event probability, ordering, artifact hash, cutoff, or marginal audit. The versioned display layer reconstructs the leading half-time result's three full-time branches from that same frozen posterior and selects one genuine representative score path per branch; callers cannot request, suppress, reorder, or hand-pick them.
