...
2. Resolve the current saved task ID, then run `lineup_scheduler.py claim --match-id <id> --thread-id <current_thread_id>`. Do not open Titan or calculate anything unless `claimed` is `true`.
...
5. If still pre-match, collect current Asian, totals, 1X2, first-half, HT/FT, confirmed lineups, and injuries. If Titan lacks either starting XI, follow [lineup-sources.md](lineup-sources.md) in the same claimed attempt: check an official competition/federation or club source first, then a public ESPN lineup page with a stable numeric `gameId`, then a Sofascore page with an exact event ID. Do not call undocumented/private APIs or automate FotMob, Flashscore, or Soccerway. Compare the result with the active archived prediction and produce visual mode output.
...
9. If collection or archival fails while the page is still pre-match, run `release --thread-id <current_thread_id> --reason <concise cause>`. A later explicit invocation may catch up before kickoff; do not start a recurring retry loop.
