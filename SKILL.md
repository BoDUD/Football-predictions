...
- Classify each HT/FT scenario as either `正式推荐` or `观察候选（未达标）`. First validate that the 3x3 matrix reproduces the displayed half-time row marginals and full-time column marginals, then use `scripts/htft_ranker.py` to select exactly two scenarios by `scenario_stability_v1`. Do not take the raw joint-probability Top 2 or highest-EV paths. EV only determines whether a stability-selected scenario clears the formal thresholds and must never choose either slot. Never hide the two scenarios behind a generic “观望” result, and never describe a negative-EV candidate as positive value.
