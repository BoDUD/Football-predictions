...
- Registered production training and formal fixed-season scoring accept only `competition_regime=regular`. In Japan J1, all 180 supplied 2026 regional-format fixtures are `2026_vision_regional`; exclude them from fitting and formal metrics, retaining only excluded-row counts as evidence of a competition-regime shift.
- The audited 9,211-match bundle supports only Brazil Serie A, Japan J1, Norway Eliteserien, and MLS. Do not transfer their development accuracy or pair-gate evidence to Korean K League or any unsupported competition. For Korean matches, use the generic workflow and keep HT/FT as observation until league-specific live-forward evidence exists.
