...
- Initial and lineup-check images use the same simple eight-column table in this exact order: `编号`, `时间`, `赛事`, `主队 vs 客队`, `主推`, `总进球`, `半全场`, `波胆`. The stage belongs in the title/subtitle. Bind every row to `archive_stage` plus `archive_version_hash`; verify kickoff, league, teams, and artifact identity so an initial card cannot leak a later lineup artifact. Derive `★` only from the unique archived formal primary, use `◇` only for a diagnostically qualified observation, and use `无正式推荐` when no direction qualified. A daily slate may have multiple starred matches but at most one starred direction per match.
- The image's `总进球` cell shows only the highest-probability goal range, its probability, and the gap over the runner-up. The `半全场` and `波胆` cells show the same jointly ranked path events in the same order: normally two pairs, or three only when the versioned distribution-complexity rule finds a divided head cluster. Each pair contains a genuine `(HT/FT × full-time score)` joint probability. Callers must not independently rank, reorder, force, or hand-fill either column.
- The compact image does not reduce the analysis. Accompanying visual-mode text or the audit must preserve complete half-time, 1X2, goal-range and BTTS distributions, opening-to-current movement, no-vig comparison, EV/edge, fundamentals/lineups, evidence coverage, and material risks. Do not expose free-form `recommendation` or `notes` as a market direction; only archived formal picks or qualified observations may state a bettable direction. If the joint artifact is missing or invalid, show `数据不足` for affected image/text fields and no fallback scenarios.
...
- Concise mode: return only the best direction, probability, EV, the system-selected two or three paired joint path events, and one short rationale. If no valid joint artifact exists, use `数据不足` instead of scenario rows. Do not add separate exact-score, HT/FT, or 0-0 audits.
...
Pass `--joint-scenario-file <joint-scenarios.json>` for the user-facing scenario contract. The file must bind the active fixture/version and validated match-path posterior, expose enough validated path data to rank genuine `(HT/FT × full-time score)` events, and include posterior/input hashes, pre-kickoff generation time, normalization, tail, convergence, and marginal-consistency audits. Its archived Top-2 audit must match the first two reconstructed events; the display layer may add the third only when the versioned complexity rule passes. `memory_store.py` must reject a mismatched or invalid file. Without an accepted joint-scenario file, the image and normal pre-match text show `数据不足`; neither archived exact-score fields nor HT/FT ranker fields are a fallback.
...
Only the final active primary affects direction accuracy, profit, and ROI. A reviewed match whose final active version has no primary must not change matches, wins/losses/pushes, stake, profit, accuracy, or ROI; persist it instead as `learning_scope: no_primary_observation` with `counts_toward_primary_record: false`, a non-empty causal learning, and include it in league/global learning samples. Frozen unconditional exact-score and HT/FT Top 2 audits may be graded diagnostically against the verified half-time/full-time result for Top-1, Top-2, Brier, log loss, and gate-funnel learning, but they remain `counts_toward_primary_record: false` with no stake, win/loss, profit, or ROI. Grade the archived user-facing joint path events separately in their original count and order, and do not reconstruct them from those marginals after the result. Keep secondary picks as pre-match references only: never settle them, record their hit/miss outcome, include them in any performance denominator, assign them a stake, or calculate profit/loss and ROI. Explain the joint-event diagnostic, internal exact-score/HTFT diagnostics, the primary result or explicit no-primary status, cumulative primary statistics, current-league primary statistics, and the saved learning. If a formal half-time/HTFT primary lacks a verified half-time score, or a corner primary lacks both official 90-minute corner counts, leave the record pending rather than inventing or partially settling it; a missing half-time score merely leaves an observation ungraded and must not block an otherwise valid no-primary/full-time review.

After `review`, `stats`, and `calibrate --write` succeed, render a deterministic review image with `python scripts/review_card_renderer.py --history <workspace>/.codex/soccer-predict/history.json --match-id <id> --output <workspace>/.codex/soccer-predict/rendered/<id>-review.png`. The image uses the same compact visual family as the initial/lineup card, binds the final active settlement basis and verified scores, shows full text without ellipsis, keeps archived paired joint-event references in their original order, and never rewrites a pre-match direction after seeing the result. Then run `plain_text_formatter.py --base-dir <workspace> --match-id <id> --kind review`. Preserve its required information once in the normal image-plus-text result; append the exact raw block only on an explicit copyable/text-only request. Always keep lines such as `主推：无正式推荐（不结算、不计战绩）`, the archived joint-event and internal score/HTFT diagnostic results, the causal learning, league record, and cumulative record even when no formal primary existed. The formatter only renders text inside Codex; it never sends messages externally.
